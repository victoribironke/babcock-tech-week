...
export default function Header({ solid = false, scrollThreshold }: { solid?: boolean; scrollThreshold?: number }) {
  const [scrolled, setScrolled] = useState(false);
  const [menuOpen, setMenuOpen] = useState(false);

  useEffect(() => {
    const handleScroll = () => {
      const heroHeight = scrollThreshold !== undefined ? scrollThreshold : window.innerHeight - 100;
  ...
  return (
    <>
      ...
      >
        <div className="max-w-[1400px] mx-auto px-6 flex items-center justify-between h-20">
          ...
          <button
            ...
            className="xl:hidden bg-transparent border-none text-white text-3xl cursor-pointer p-1 z-[1100]"
          >
            {menuOpen ? "✕" : "☰"}
          </button>
      ...
      {/* Backdrop */}
      <div
        className={`fixed inset-0 bg-black/50 z-[1050] xl:hidden transition-opacity duration-300 ${
      ...
      {/* Panel */}
      <div
        className={`fixed top-0 right-0 h-full w-[85%] max-w-[360px] bg-white z-[1060] xl:hidden transition-transform duration-300 ease-in-out shadow-2xl ${
      ...
      >
        ...
        <div className="flex items-center justify-end px-6 h-20">
          <button
            ...
            className="text-black text-3xl bg-transparent border-none cursor-pointer p-1"
          >
            ✕
          </button>
        </div>
        ...
        <nav className="flex flex-col px-8 gap-1">
          {navLinks.map((link) => (
            <Link
              key={link.label}
              href={link.href}
              className="text-[13px] font-bold tracking-wider text-black/80 flex items-center justify-between py-4 border-b border-black/8 hover:text-black transition-colors"
              onClick={() => setMenuOpen(false)}
            >
              {link.label}
        ...
        {/* Social Icons */}
        <div className="flex gap-5 px-8 mt-10">
          {socialLinks.map((social) => (
            <a
              key={social.name}
              href={social.href}
